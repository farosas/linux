...
long kvmhv_enter_nested_guest(struct kvm_vcpu *vcpu)
{
	...
	/*
	 * L0 issues hcalls to L1 via H_ENTER_NESTED return path and
	 * receives the results back during H_ENTER_NESTED entry
	 * (here), so we might need to skip entering the guest to
	 * handle the hcall result.
	 */
...
{
	if (!kvmhv_on_pseries()) {
		printk(KERN_DEBUG "%s powernv\n", __func__);
		mmu_partition_table_set_entry(lpid, dw0, dw1, false);
		return;
	}
...
//		printk_ratelimited(KERN_DEBUG "return ret=%ld n_gpa=%#lx\n", ret, n_gpa);
	...
	/* Falied to set the reference/change bits */
